defmodule UiGraph.Relay.Connection.Options do
  @moduledoc false
...
defmodule UiGraph.Relay.Connection do
  @moduledoc """
    ...
    connection field :pets, node_type: :pet do
      resolve fn
        pagination_args, %{source: person} ->
          UiGraph.Relay.Connection.from_list(
  ...
  `UiGraph.Relay.Connection.from_list/2`, which takes a list and the pagination
  ...
  Note: `UiGraph.Relay.Connection.from_list/2`, like `connectionFromArray` in
  ...
  `UiGraph.Relay.Connection.Notation`.
  """

  alias UiGraph.Relay.Connection.Options
  ...
  def cursor_to_offset(cursor) when is_binary(cursor) do
    with {:ok, @cursor_prefix <> raw} <- Base.url_decode64(cursor, padding: false),
         {parsed, _} <- Integer.parse(raw) do
      parsed
    end
  end
  def cursor_to_offset(_) do
    :error
  end
