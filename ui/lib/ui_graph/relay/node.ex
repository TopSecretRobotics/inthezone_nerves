defmodule UiGraph.Relay.Node do
  @moduledoc """
  ...
  def resolve_with_global_id(%{state: :unresolved} = res, _) do
    ...
    {:ok, result} <- UiGraph.Relay.Node.from_global_id(global_id, res.schema) do
  ...
  def from_global_id(global_id, schema) do
    with {:ok, global_id} <- Base.url_decode64(global_id, padding: false) do
      global_id
      |> :binary.split(<< 0 >>, [:trim_all])
      |> do_from_global_id(global_id, schema)
    else _ ->
