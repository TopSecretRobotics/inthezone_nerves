defmodule UiGraph.Relay.Node.ParseIDs do
  ...
  field :item, :item do
    ...
    middleware UiGraph.Relay.Node.ParseIDs, item_id: :item
  ...
  field :foo, :foo do
    ...
    middleware UiGraph.Relay.Node.ParseIDs, interface_id: [:item, :thing]
  ...
  mutation do
    payload field :update_parent do
      ...
      middleware UiGraph.Relay.Node.ParseIDs, parent: [
        id: :parent,
        children: [id: :child],
        child: [id: :child]
      ]
  ...
  defmodule MyApp.Schema do
    ...
    def middleware(middleware, _, %Absinthe.Type.Object{identifier: :query}) do
      [{UiGraph.Relay.Node.ParseIDs, @node_id_rules} | middleware]
    end
  ...
  (defined using the `UiGraph.Relay.Mutation` macros) _before_ the
  `UiGraph.Relay.Mutation` middleware, you need to include a wrapping
  ...
  mutation do
    ...
    payload field :change_something do
      ...
      # ...
      middleware UiGraph.Relay.Node.ParseIDs, profile: [
        user_id: :user
     ]
  ...
  def middleware(middleware, %Absinthe.Type.Field{identifier: :change_something}, _) do
    ...
    [{UiGraph.Relay.Node.ParseIDs, input: [profile: [user_id: :user]]} | middleware]
  end
  def middleware(middleware, _, _) do
    middleware
  end
  ...
  list and insert `UiGraph.Relay.Node.ParseIDs` _after_
  `UiGraph.Relay.Mutation`, you don't include the wrapping `:input`.

  ## Compatibility Note for Middleware Developers

  If you're defining a piece of middleware that modifies field
  arguments similar to `UiGraph.Relay.Mutation` does (stripping the
  outer `input` argument), you need to set the private
  `:__parse_ids_root` so that this middleware can find the root schema
  node used to apply its configuration. See `UiGraph.Relay.Mutation`
  ...
  # To support middleware that may run earlier and strip away toplevel arguments (eg, `UiGraph.Relay.Mutation` stripping
  ...
  defp process(%Rule{} = rule, arg_value, resolution, _schema_node, errors) do
    with {:ok, node_id} <- UiGraph.Relay.Node.from_global_id(arg_value, resolution.schema),
