defmodule UiGraph.Relay.Node.Notation do

  @moduledoc """
  Macros used to define Node-related schema entities

  See `UiGraph.Relay.Node` for examples of use.
  """
  ...
  See the `UiGraph.Relay.Node` module documentation for examples.
  """
  ...
  defp field_body do
    quote do
      ...
      middleware {UiGraph.Relay.Node, :resolve_with_global_id}
    end
  end
  ...
  defp object_body(name, id_fetcher) do
    quote do
      ...
      field :id, non_null(:id) do
        resolve UiGraph.Relay.Node.global_id_resolver(unquote(name), unquote(id_fetcher))
      end
      interface :node
    end
  end
